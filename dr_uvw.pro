...
;     Will also mark with an asterisk those radial velocities that give UVW within the (modified) "good UVW box" for 
;     young stars as defined by Zuckerman & Song ARAA 2004, 42, 685
; CALLING SEQUENCE:
;     DR_UVW, RA, DEC, DIST, PMRA, PMDEC, [/PMCOS, RVMIN=RVMIN, RVMAX=RVMAX, RVSTEP=RVSTEP, LOWRV=LOWRV, HIGHRV=HIGHRV, /DISP]
; OUTPUT:
...
;     /DISP   Print out results on screen? (Default: no)
; OPTIONAL OUTPUT:
;     LOWRV   Lowest radial velocity that matches good UVW box
;     HIGHRV  Highest radial velocity that matches good UVW box
; METHOD:
...
;      Modified good UVW box: U goes as far as -25	February 2011
;-

PRO DR_UVW, RA, DEC, DIST, PMRA, PMDEC, PMCOS=PMCOS, RVMIN=RVMIN, RVMAX=RVMAX, RVSTEP=RVSTEP, lowrv=lowrv, highrv=highrv, disp=disp1

ON_ERROR, 2

IF N_PARAMS() LT 5 THEN BEGIN
  print, 'Syntax - DR_UVW, RA, DEC, DIST, PMRA, PMDEC, /PMCOS, RVMIN=RVMIN, RVMAX=RVMAX, RVSTEP=RVSTEP, LOWRV=LOWRV, HIGHRV=HIGHRV, /DISP
  RETURN
ENDIF
...
IF n_elements(rvstep) EQ 0 AND ~arg_present(rvstep) THEN rvstep = 1.

disp=0
IF keyword_set(disp1) THEN disp=1
...
FOR i=0, len-1 DO BEGIN
  ...
  IF ( (U GE -25) AND (U LE 0) AND (V GE -34) AND (V LE -10) AND (W GE -20) AND (W LE 3) ) THEN BEGIN
    IF disp THEN print, rv[i], u, v, w, ' *'
    if (gd lt 1) then begin
      lowrv=rv[i]
      gd = gd+1
    endif
    highrv=rv[i]
